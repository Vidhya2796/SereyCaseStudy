...
System.out.println(response.getBody());
	...
	@PostMapping("/quickr/users/retrieve")
	public UserLoginAccount retrievePassword(@RequestBody UserLoginAccount userLoginAccount) {
		System.out.println(userLoginAccount);
		RestTemplate restTemplate = restTemplateBuilder.build();
		/*InstanceInfo instanceInfo = client.getNextServerFromEureka("producer", false);*/
...
System.out.println(response.getBody());
		List<UserRegistrationAccount> users=response.getBody();
		for (UserRegistrationAccount user : users) {
			if(user.getEmail().equals(userLoginAccount.getEmail())) {
				System.out.println(user.getPassword());
				userLoginAccount.setPassword(user.getPassword());
